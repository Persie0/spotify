...
The factory path is also concrete: `TimelineConductorSetupImpl::factory` allocates `0xf0` bytes at `0x135cc69`, passes that object as the first argument to `0x135d056` at `0x135ce8b`, and finally stores the resulting object pointer into the factory output at `0x135cebf`. Therefore the next class-identification step is restricted to the object initialized by `0x135d056`. The nearby address point `0x1867b28` is **not** promoted merely because it is close to the provider vtable: its materializer at `0x135a7d3` allocates a different `0xe0`-byte object and its `+0x28` returns embedded `this+0x50`, so it must be treated as a separate class unless the real TimelineConductor constructor proves otherwise.
