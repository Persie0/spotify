...
| In-process `SignalCommand("skip-ad")` / patched Orbit / LSPosed hook | **Viable with root — specified, not built** | Fire `hrw.mo48412a(new b8p0(SignalCommand.create("skip-ad")))` iff ad playing + `PlayerState.signals()` contains `"skip-ad"` (protocol-identical to a tap). See `analysis/adblock-inprocess-hook-spec.md`. Risks: `librootChecker.so`, version drift, account telemetry |
