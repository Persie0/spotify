...
| H3 | Ad check: current `ContextTrack` metadata `is_advertisement=true` (internal twin of the exported `ADVERTISEMENT` flag, `er3.java:436`) | Fire only during real ads | `p4h1.smali:1735-1751`: `EsProvidedTrack → n9k.a() → ContextTrack → Builder.track()`; `AutoValue_PlayerState.track():xul0`, unwrap `b()` (=get; `c()`=isPresent per decompiled `p.xul0`), `ContextTrack.metadata():xf40` (a `java.util.Map`, DEX-verified) → `get("is_advertisement")`. Full chain verified, no guessing |
